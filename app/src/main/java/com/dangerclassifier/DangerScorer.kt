...
object DangerScorer {
    ...
    fun getDangerScore(label: String): Int = dangerMap[label.lowercase()] ?: 0

    fun analyze(boxes: List<DetectionBox>): DangerResult {
        if (boxes.isEmpty()) {
            return DangerResult(
                score = 0, level = "SAFE",
        ...
        val detections = boxes.map { Pair(it.label, it.confidence) }

        val weighted = detections.map { (label, conf) ->
            Triple(label, conf, (dangerMap[label.lowercase()] ?: 0) * conf)
        }.sortedByDescending { it.third }

        val topWeighted = weighted.maxOf { it.third }
        var score = (topWeighted / 9f * 10).toInt().coerceIn(0, 10)

        val highDangerCount = boxes.count { it.dangerScore >= 5 && it.confidence > 0.4f }
        ...
        if (boxes.count { it.label.lowercase() in vehicleLabels } >= 3)
            score = (score + 1).coerceAtMost(10)

        val level = toLevel(score)
        val reasoning = buildReasoning(detections, weighted, score)

        // Top 3 boxes worth highlighting (danger >= 4, confidence >= 40%)
    ...
    private fun buildReasoning(
        ...
        val topItems = detections.take(5).joinToString(", ") { (l, c) -> "$l (${(c*100).toInt()}%)" }
